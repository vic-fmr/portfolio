...
- Não invente projetos ou qualificações que não estejam descritos aqui. Use formatação Markdown (negrito, listas, blocos de código se pedirem um exemplo de código React/Node.js) para deixar as respostas ricas e estruturadas.
`;
...
async function startServer() {
  ...
  // Lazy initialize GoogleGenAI. Falls back gracefully if key is missing.
  let ai: GoogleGenAI | null = null;
  function getAI() {
    if (!ai) {
      const apiKey = process.env.GEMINI_API_KEY;
      if (!apiKey) {
        console.warn("Aviso: GEMINI_API_KEY não foi encontrada no ambiente. Algumas funções de IA estarão indisponíveis.");
        return null;
      }
      ai = new GoogleGenAI({
        apiKey,
  ...
  app.post("/api/chat", async (req, res) => {
    try {
      ...
      // Convert local message format to `@google/genai` generateContent format
      // Local message format: { id: string, sender: "user" | "bot", text: string, timestamp: string }
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`[Server] Portfolio running at http://localhost:${PORT} in ${process.env.NODE_ENV || "development"} mode`);
  });
}
